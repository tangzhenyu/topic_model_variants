...
void model::initializeHists_doc(int ndMax)
{
	topicDocHist = new int*[K];
	for(unsigned k = 0 ; k < K ; ++k)
	{
		topicDocHist[k] = new int[ndMax+1]();
	}
	topicDocNormHist = new int[ndMax+1]();
	for(unsigned m = 0 ; m < M ; ++m)
	{
		topicDocNormHist[topicDocCountsNorm[m]]++;
	}
}	

void model::initializeHists_word(int *wordCounts)
{
	wordTopicHist = new int*[V];
	for(unsigned v = 0 ; v < V ; ++v)
	{
		wordTopicHist[v] = new int[wordCounts[v] + 1]();
		std::cout<<v<<"<==>"<<wordCounts[v]<<std::endl;
		//std::cout<<wordCounts[v] + 1<<std::endl;
		for(unsigned k = 0 ; k < K ; ++k)
		{
			wordTopicHist[v][wordTopicCounts[v][k]]++;
		}
	}
}	
void model::optimizeParam_alpha(int numIters)
{
	...
	for (int j=0; j<K; j++) {//for all topics
		...
		//topicDocCounts[j][d]=the number of appearance of topic j in doc d
...
void model::optimizeParam_beta(int numIters)
{
//	assert(wordTopicHist != NULL);
	...
	for (int k=0; k<K; k++)
		wordTopicNormHist[wordTopicCountsNorm[k]]++;
	for (int i=0; i<numIters; i++) {

		double denominator = 0.0;
		double currentDigamma = 0.0;

		for (int n=1; n < (njMax+1); n++) {
		...
		for (int w=0; w < V; w++) {
			...
			double oldParam = beta_word[w];
			beta_word[w] = 0.0;
			currentDigamma = 0;

			for (int n=1; n < (wordCounts[w] + 1); n++) {
				currentDigamma += 1.0 / (oldParam + n - 1);
				beta_word[w] += wordTopicHist[w][n] * currentDigamma;
			}

			beta_word[w] *= oldParam / denominator;//update beta of word w

			if (beta_word[w] == 0.0)
				beta_word[w] = 1e-10;

			beta_word_sum += beta_word[w];
		}
	}
...
int model::parse_args(std::vector<std::string> arguments)
{
	for (auto arg = arguments.begin(); arg != arguments.end(); ++arg)
	{
		...
		else if (*arg == "--num-threads")
		{
			...
			if(_nt > 0)
			{
				ntt = num_table_threads((unsigned)_nt);//ntt = _nt / 16;
				nst = _nt - ntt;
			}
		}
	}
...
void model::init_alpha_topic()
{
	//initialize alpha array
	alpha_topic = new double[M];
	for(unsigned k = 0 ; k < K ; ++k)
	{
		alpha_topic[k] = alphaK;
	}
	alpha_topic_sum = alphaK * K;
	//initialize counts
	topicDocCounts = new int*[K];
	for(unsigned k = 0 ; k < K ; ++k)
	{
		topicDocCounts[k] = new int[M]();
	}
	

	topicDocCountsNorm = new int[M]();
	for(int m = 0 ; m < M ; ++m)
	{
		for (const auto& k : n_mks[m])
		{   
			topicDocCounts[k.idx][m]=k.val;
//			rev_mapper[k.idx] = kc++;
		}
		topicDocCountsNorm[m]=trngdata->docs[m]->length;	
	}
	//initialize hists
	initializeHists_doc(ndMax);	
}

void model::init_beta_word()
{
	//initialize beta array
	beta_word = new double[V];
	for(unsigned v = 0 ; v < V ; ++v)
	{
		beta_word[v] = beta;
	}
	beta_word_sum = beta * V;

	//initialize wordTopicCounts
	wordTopicCounts = new int*[V];
	for(unsigned v = 0 ; v < V ; ++v)
	{
		wordTopicCounts[v] = new int[K]();
		for(int k = 0 ; k < K ; ++k)
		{
			wordTopicCounts[v][k]=n_wk[v][k];
		}
	}

	//initialize wordTopicCountsNorm
	wordTopicCountsNorm = new int[K]();
	for(int k = 0 ; k < K ; ++k)
	{
		wordTopicCountsNorm[k]=n_k[k];
	}
	//initialize hists
	initializeHists_word(wordCounts);	
}
...
int model::init_train()
{
	...
	std::cout<<"Initialize alpha topic!"<<std::endl;
	init_alpha_topic();
	std::cout<<"Initialize beta word!"<<std::endl;
	init_beta_word();
...
int model::save_model(unsigned iter) const
{
	...
	save_model_tassgin(mdir + model_name + ".tassgin");
...
int model::save_model_tassgin(std::string filename) const
{
	int m,i;
	...
	std::stringstream ss;
	for (unsigned m = i; m < M; ++m)
	{
		for (unsigned n = 0; n < trngdata->docs[m]->length; ++n)
		{   
			unsigned w = trngdata->docs[m]->words[n];
			unsigned short topic = z[m][n];
			ss<<w<<":"<<topic<<" ";	
		}
