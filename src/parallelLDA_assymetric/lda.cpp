...
int sparseLDA::specific_init()
{
	...
	//smtx_docs = new shared_mutex[M];
	smtx_update_assymmetric = new shared_mutex();
	//mutex_update_assymmetric = new mutex();
	symmetric_sum = ntt;//threads number for update model
...
int sparseLDA::sampling(unsigned i)
{
    ...
    for (unsigned short k = 0; k < K; ++k)
    {
            ...
            q1[k] = alpha_topic[k] / (nwsum_local[k] + beta_word_sum);
			//ssum += 1 / (nwsum_local[k] + Vbeta);
            ssum += 1 / (nwsum_local[k] + beta_word_sum);
    }

    //ssum *= alphaK * beta;//G
    ssum *= (alpha_topic_sum / K) * (beta_word_sum / V);//Go

    std::chrono::high_resolution_clock::time_point ts, tn;

    unsigned iter; current_iter = &iter;	
	std::cout<<"Begin to sample!!"<<beta_word_sum<<std::endl;
    for (iter = 0; iter < n_iters; ++iter)
    {
        ts = std::chrono::high_resolution_clock::now();
        // for each document of worker i
        for (unsigned m = i; m < M; m+=nst)
        {
            ...
            if (m % 10 == i)
            {
                ...
                //ssum *= alphaK * beta;
                ssum *= (alpha_topic_sum / K ) * (beta_word_sum / V);
            }
            ...
            for (const auto& k : n_mks[m])
            {
                    ...
                    double temp = k.val / (nwsum_local[k.idx] + beta_word_sum);
                    rsum += temp;
                    q1[k.idx] += temp;
            }
            rsum *= beta / V;
            ...
            {	
				...
				//smtx_docs[m].lock();
				decrementCounts_alpha_topic(old_topic,m);
				//smtx_docs[m].unlock();
				std::cout<<"Decrement alpha topic"<<std::endl;
				//decrement for updating params
				/*
				smtx[w].lock_shared();
				decrementCounts_beta_word(w,old_topic,init);
				decrementCounts_alpha_topic(w,m);
				...
				*/
				...
				//ssum==G ; rsum ==>F ; qsum==>E	

                // update the bucket sums
                unsigned nd_mk = n_mks[m].decrement(rev_mapper[topic]);
                //double denom = nwsum_local[topic] + Vbeta;
                double denom = nwsum_local[topic] + beta_word_sum;

                //ssum -= (alphaK * beta) / (denom + 1);
                //ssum += (alphaK * beta) / denom;
				std::cout<<"beta_word "<<w<<":"<<beta_word[w]<<std::endl;
                ssum -= (alpha_topic[topic] * beta_word[w]) / (denom + 1);
                ssum += (alpha_topic[topic] * beta_word[w]) / denom;

                //rsum -= (beta + nd_mk * beta) / (denom + 1);
                //rsum += (nd_mk * beta) / denom;
                rsum -= (beta_word[w] + nd_mk * beta_word[w]) / (denom + 1);
                rsum += (nd_mk * beta_word[w]) / denom;
                //rsum -= beta / denom;
				
                //q1[topic] = (alphaK + nd_mk) / denom;
                q1[topic] = (alpha_topic[topic] + nd_mk) / denom;

                //  Divide the full sampling mass into three buckets. [s | r | q]
                qsum = 0;
                unsigned short ii=0;
                smtx[w].lock_shared();
                for (const auto &k : nws[w])//k.val:topic word count    k.idx:topic id
                {
                    fwd_mapper[ii] = k.idx;
                    p[ii] = k.val*q1[k.idx];//e(k) == p[k]
                    qsum += p[ii++];
                }
                smtx[w].unlock_shared();
                ...
                if (u < ssum)	//smoothing only bucket
                {
                        ...
                        //u /= alphaK * beta;
                        u /= alpha_topic[topic] * beta_word[w];
                        //for (topic = -1; u > 0; u -= 1 / (nwsum_local[++topic] + Vbeta));
                        for (topic = -1; u > 0; u -= 1 / (nwsum_local[++topic] + beta_word_sum));
                }
                else if (u < ssum + rsum) //document topic bucket
                {
                    ...
                    for (const auto& k : n_mks[m])
                    {
                        ...
                        u -= k.val / (nwsum_local[k.idx] + beta_word_sum);
                        if (u <= 0)
                        {
                            topic = k.idx;
                            break;
                ...
                nwsum_local[topic] += 1;
				//smtx_docs[m].lock();
				incrementCounts_alpha_topic(topic, m);
				//smtx_docs[m].unlock();
				//increment for updating params
				/*
				smtx[w].lock_shared();
				incrementCounts_beta_word(w, topic, !init);
				incrementCounts_alpha_topic(topic, m);
				smtx[w].unlock_shared();
				*/
                ...
                denom = nwsum_local[topic] + beta_word_sum;
                ...
                if (topic!=old_topic)
                {
                    if(rev_mapper[topic] == K)
                    {
                        rev_mapper[topic] = n_mks[m].push_back(topic, 1);
                        //rsum += beta / denom;
                        rsum += beta_word[w] / denom;

                        //q1[topic] = (alphaK + 1) / denom;
                        q1[topic] = (alpha_topic[topic] + 1) / denom;
                    }
                    else
                    {
                        ...
                        //rsum += (nd_mk * beta) / denom;
                        rsum -= (nd_mk * beta_word[w] - beta_word[w]) / (denom - 1);
                        rsum += (nd_mk * beta_word[w]) / denom;
						//q1[topic] = (alphaK + nd_mk) / denom;
						q1[topic] = (alpha_topic[topic] + nd_mk) / denom;
                    }
            ...
            for (const auto& k : n_mks[m])
            {
                ...
                q1[k.idx] -= k.val / (nwsum_local[k.idx] + beta_word_sum);
                //nd_m[k.idx] = 0;
                rev_mapper[k.idx] = K;
            }
        }
		...
		//exit(-1);
		//smtx_update_assymmetric->lock();
		//mutex_update_assymmetric.lock();
		update_param.lock();
		std::cout<<"Symmetric_lock:"<<symmetric_sum<<std::endl;
		symmetric_sum--;
		if(symmetric_sum == 0){
			std::cout<<"Symmetric_sum!!"<<symmetric_sum<<std::endl;
			std::cout<<"Update beta word!!"<<std::endl;
			std::cout<<"Update alpha topic!!"<<std::endl;
			updater_beta_word(5);
			exit(-1);
			updater_alpha_topic(5);
			symmetric_sum = ntt;
			std::cout<<"Symmetric_lock:"<<symmetric_sum<<std::endl;
			if(init){
				init=false;
		...
		update_param.unlock();
		//smtx_update_assymmetric->unlock();
		//mutex_update_assymmetric.unlock();
        //std::cout << "In thread " << i << " at iteration " << iter << " ..." 
        //                  << "Time: " << std::chrono::duration_cast<std::chrono::milliseconds>(tn - ts).count() << std::endl;
    }
    ...
    delete[] rev_mapper;
    delete[] fwd_mapper;
	delete smtx_update_assymmetric;
    current_iter = NULL;
    ...
    do
    {
        for(unsigned tn = 0; tn<nst; ++tn)
        {
            if(!cbuff[i*nst + tn].empty())
            {
				...
				decrementCounts_beta_word(temp.word,temp.old_topic,!init);
                //decrementCounts_alpha_topic(w,m);

				incrementCounts_beta_word(temp.word, temp.new_topic, !init);
				//incrementCounts_alpha_topic(tmp.new_topic, m);
