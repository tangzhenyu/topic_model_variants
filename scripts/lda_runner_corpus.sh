...
do
	...
	dist/parallelLDA --method "$METHOD" --testing-mode net --alpha 50 --beta 0.1 --num-threads $NT --num-topics $NUM_TOPICS --num-iterations $NUM_ITER --output-state-interval 1 --output-model $DIR_NAME --num-top-words 15 --dataset /data6/data/"$DATASET" | tee -a $DIR_NAME/log.txt
